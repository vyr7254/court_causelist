...
import shutil   # ✅ added for backup copy

# ------------------------------
# CONFIG
# ------------------------------
YEAR = 2025
BASE_FOLDER = r"D:\banglorehighcourt\bengaluru_causelist"
OUTPUT_EXCEL = os.path.join(BASE_FOLDER, f"Bengaluru_bench_from7jun{YEAR}.xlsx")
PROGRESS_FILE = os.path.join(BASE_FOLDER, f"progress_bengaluru_from7jun{YEAR}.json")
...
# ------------------------------
# Improved party & advocate parsing (updated)
# ------------------------------
def split_party_and_advocate(text, prefix=None):
    ...
    # Handle R1-/R2- respondent style
    if re.search(r'\bR\d+\s*[-–]\s*', t):
        parties = re.findall(r'R\d+\s*[-–]\s*([^R]+?)(?=\s*R\d+\s*[-–]|$)', t)
        party_text = "; ".join([p.strip(" -") for p in parties])
        advs = re.findall(r'FOR\s+R\d+\s*([^,;]*)', t, flags=re.I)
        adv_text = "; ".join([a.strip() for a in advs])
        return party_text, adv_text

    # Handle FOR R1, FOR R2 patterns
    if "FOR R" in t.upper():
        parts = re.split(r'\bFOR\s+R\d+', t, flags=re.I)
        party = parts[0].strip()
        adv = " ".join(parts[1:]).strip()
        return party, adv

    # Fallback: original logic
    parts = re.split(r'\b(GA|SD|ADV|ADVOCATE)\b', t, flags=re.I)
    if len(parts) > 1:
        return parts[0].strip(), " ".join(parts[1:]).strip()
    return t, ""

# ------------------------------
# Page parsing of table rows
# ------------------------------
            ...
            try:
                ...
                for row in rows[header_row+1:] if header_row is not None else rows[1:]:
                    try:
                        ...
                        # ✅ FIXED: Extract Sl.No / SI.No / Sl No (Causelist_Slno) correctly
                        causelist_slno = ""
                        try:
    # First try using detected header index
                           if "sl_no" in header_indices and header_indices["sl_no"] < len(cell_texts):
                             causelist_slno = cell_texts[header_indices["sl_no"]].strip()
                           else:
        # Fallback: assume first column is Sl.No.
                              if len(cell_texts) > 0:
                                causelist_slno = cell_texts[0].strip()
                        except Exception:
                             causelist_slno = ""

# Clean up inconsistent text and ensure a number is captured
                        if not causelist_slno or not re.search(r'\d+', causelist_slno):
                            first_col = cell_texts[0].strip() if cell_texts else ""
                            match = re.search(r'(\d+)', first_col)
                            if match:
                                causelist_slno = match.group(1)
...
def autosave_records(records):
    ...
    try:
        ...
        combined.drop_duplicates(subset=["Bench","Cause_Date","Court_Hall","Case_Type","Case_No","Year"], inplace=True)
        ...
        # ✅ Backup every 10 days based on current date
        today = datetime.now().date()
        backup_tag = (today - datetime(YEAR, 1, 1).date()).days // BACKUP_INTERVAL_DAYS
        backup_file = os.path.join(BACKUP_FOLDER, f"backup_{YEAR}_{backup_tag:02d}.xlsx")
        if not os.path.exists(backup_file):
            shutil.copy2(OUTPUT_EXCEL, backup_file)
            debug_print(f"🗂️ Backup created: {backup_file}")
...
def main():
    ...
    # ✅ Start from 01 jan 2025
    start_date = datetime(YEAR, 6, 7)
